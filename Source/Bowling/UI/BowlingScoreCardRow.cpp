...
void UBowlingScoreCardRow::SyncWithPlayerState(TObjectPtr<ABowlingPlayerState> playerState)
{
	...
	for(int i = 0; i < playerState->Frames.Num(); ++i)
	{
		...
		if(i == ABowlingGameModeBase::GetFinalFrame(GetWorld()) - 1)
		{
			...
			bool ball2WasSpare = currentFrame.ball1Pins != ABowlingGameModeBase::GetNumPins(GetWorld()) &&
				currentFrame.ball1Pins + currentFrame.ball2Pins == ABowlingGameModeBase::GetNumPins(GetWorld());
			FinalEntry->SetBall2(currentFrame.ball2Pins,
			                     ball2WasSpare);
			
			bool ball3WasSpare = currentFrame.ball2Pins != ABowlingGameModeBase::GetNumPins(GetWorld()) &&
				currentFrame.ball2Pins + currentFrame.ball3Pins == ABowlingGameModeBase::GetNumPins(GetWorld());
